"""
DAG to monitor Snowflake for new swap data and process token activity.
"""
...
from airflow.providers.snowflake.sensors.snowflake import SnowflakeTableSensor
...
@dag(
    dag_id='snowflake_driven_token_activity',
    default_args=default_args,
    description='Monitor Snowflake for new data and process token activity',
    schedule_interval='*/3 * * * *',  # Run every 3 minutes
    start_date=datetime(2025, 2, 1),
    catchup=False,
    tags=['token', 'notification', 'snowflake', 'slack'],
)
def snowflake_driven_token_activity():
    """
    DAG that monitors Snowflake for new data and processes it.
    """
    
    # 1. Check for new data in Snowflake from the last run
    # This query looks for records newer than the last execution
    check_for_new_data = SnowflakeTableSensor(
        ...
        WHERE TIMESTAMP > DATEADD(minute, -{{ var.get('token_activity_last_check_minutes', 5) }}, CURRENT_TIMESTAMP())
        """,
        timeout=60 * 5,  # 5 minute timeout
        poke_interval=30,  # Check every 30 seconds
        mode='reschedule',  # Reschedule to free up workers while waiting
        soft_fail=True  # Continue DAG execution if no data found
    )
    
    # 2. Update the last check timestamp variable
    @task(task_id="update_last_check_time")
    def update_last_check_time():
        """Update the timestamp of the last successful check"""
        from airflow.models import Variable
        
        # Current time in minutes since epoch for easy comparison
        current_minutes = int(datetime.now().timestamp() / 60)
        Variable.set('token_activity_last_check_minutes', 5)  # Reset to 5 minutes for next run
        
        logger.info(f"Updated last check time stamp")
        return True
    ...
    def check_whale_notifications(dag_interval: int = 10) -> list:
        ...
        # Query template for token activity analysis
        # Your existing query from the original code
        ...
        # Process each time interval - your existing code from here on
    ...
    def store_notifications(candidates: list) -> str:
        """
        Process notification candidates and store them.
        This is your existing code from the original DAG, unmodified.
        """
        # Using your existing store_notifications function - no changes needed
        # This keeps all your filtering, token metadata lookup, and other logic
        
        # For brevity, I'm not repeating the entire function here since it would be the same
        # as in your original DAG. You can use that function as-is.
    ...
    # Define task flow for the DAG
    update_time = update_last_check_time()
    
    # Only proceed with the DAG if new data is found
    check_for_new_data >> update_time
    
    candidates = check_whale_notifications()
    update_time >> candidates
    ...
    candidates >> notifications_message >> branch
    branch >> [send_notification, skip_notification]

# Instantiate the DAG
snowflake_dag = snowflake_driven_token_activity()